...
async function enrichChunkWithOpenAI(transactionsChunk, chunkNumber, totalChunks) {
  ...
  console.log(`Sending request to OpenAI for chunk ${chunkNumber}/${totalChunks}...`);
  ...
  try {
    ...
    console.log(`Received response from OpenAI for chunk ${chunkNumber}/${totalChunks}`);
    ...
    console.log(`Successfully parsed response with ${parsed.results.length} transactions`);
...
async function main() {
  try {
    ...
    const outputPath = path.join(__dirname, "enriched_debug.json");
